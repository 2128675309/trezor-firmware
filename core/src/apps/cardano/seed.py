...
def derive_and_store_secrets(passphrase: str) -> None:
    ...
    if words_count == 24:
        icarus_trezor_secret = mnemonic.derive_cardano_icarus(passphrase, trezor_derivation=True)
    else:
...
async def _get_keychain_bip39(ctx: wire.Context, derivation_type: CardanoDerivationType) -> Keychain:
...
async def get_keychain(ctx: wire.Context, derivation_type: CardanoDerivationType) -> Keychain:
